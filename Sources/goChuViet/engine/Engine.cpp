 ...
 * first 2 myByte_t is character code or key code.
...
static myByte_t vowelCount = 0;
static myByte_t vowelStartIndex = 0;
static myByte_t vowelEndIndex = 0;
static myByte_t vowelWillSetMark = 0;
//static int i = 0;
//static int ii = 0;
//static int iii = 0;
//static int j=0;
...
static myByte_t _upperCaseStatus = 0; //for Write upper case for the first letter; 2: will upper case
...
static std::wstring string_to_wide_string(const std::string& aString)
{
    if (aString.empty())
    {
        return L"";
    ...
    const auto size_needed = MultiByteToWideChar(CP_UTF8, 0, &aString.at(0), (int)aString.size(), nullptr, 0);
    if (size_needed <= 0)
    {
    ...
    MultiByteToWideChar(CP_UTF8, 0, &aString.at(0), (int)aString.size(), &result.at(0), size_needed);
    return result;
}
...
std::string wideStringToUtf8(const std::wstring& str) {
    ...
    return wide_string_to_string(str);
}
...
static bool isWordBreak(const vKeyEvent& event, const vKeyEventState& state, const Uint16& data, const eCapStatus &capStatus) {
    if ( event == vKeyEvent::Mouse)
        return true;

    static unsigned int _breakCode[] = {
    KEY_ENTER, KEY_COMMA, KEY_DOT, KEY_SEMICOLON, KEY_LEFT, KEY_ESC, KEY_TAB,  KEY_RETURN,  KEY_RIGHT, KEY_DOWN, KEY_UP,
    KEY_SLASH, KEY_QUOTE, KEY_BACK_SLASH, KEY_MINUS, KEY_EQUALS, KEY_BACKQUOTE, KEY_TAB
#if _WIN32
    ...
    for (size_t i = 0; i < sizeof(_breakCode)/sizeof(unsigned int); ++i) {
        if (_breakCode[i] == data) {
            return true;
        }
    }

    if (data == KEY_SLASH && capStatus == eShift)
        return true;
...
static void setKeyData(const myByte_t& index, const Uint16& keyCode, const bool& isCaps) {
    if (index < 0 || index >= MAX_BUFF)
        return;
    TypingWord[index] = keyCode | (isCaps ? CAPS_MASK : 0);
}

const wchar_t* sac = L"sac"; 
const wchar_t* huyen = L"huyen";
const wchar_t* hoi = L"hoi"; 
const wchar_t *nga = L"nga"; 
const wchar_t* nang = L"nang";
const wchar_t* khongdau = L"0dau";

const wchar_t* getToneMarkDesc(Uint32 w) {
  switch (w&MARK_MASK) {
    case MARK1_DAU_SAC_MASK:return sac;
    case MARK2_DAU_HUYEN_MASK: return huyen;
    case MARK3_DAU_HOI_MASK:return hoi;
    case MARK4_DAU_NGA_MASK:return nga;
    case MARK5_DAU_NANG_MASK:return nang;
    default: return khongdau;
  }
}

static void checkSpelling(const bool& forceCheckVowel=false) {    
    bool _spellingOK = false;  //_spellingOK = false;
    bool _spellingFlag = false;
    bool _spellingVowelOK = false;
    _spellingVowelOK = true;
    Uint32 _spellingEndIndex = 0;
    ...
    if (_spellingEndIndex > 0) {
        ...
        if (IS_CONSONANT(CHR(0))) {
            for (size_t phuam = 0; phuam < _consonantTable.size(); ++phuam) {
                _spellingFlag = false;
                if (_spellingEndIndex < _consonantTable[phuam].size())
                    _spellingFlag = true;
                for (j = 0; j < _consonantTable[phuam].size(); ++j) {
                    if (_spellingEndIndex > j &&
                        (_consonantTable[phuam][j] & ~(vQuickStartConsonant ? END_CONSONANT_MASK : 0)) != CHR(j) &&
                        (_consonantTable[phuam][j] & ~(vAllowConsonantZFWJ ? CONSONANT_ALLOW_MASK : 0)) != CHR(j)) {
                        _spellingFlag = true;
                        break;
                    }
                }
        ...
        if (k > j) { //has vowel,
            ...
            if (k - j > 1 && forceCheckVowel) {
                ...
                for (int l = 0; l < vowelSet.size(); l++) {
                    _spellingFlag = false;
                    int ii = 1;
                    for (ii = 1; ii < vowelSet[l].size(); ii++) {
                        if (j + ii - 1 < _spellingEndIndex 
                            && vowelSet[l][ii] != ( (   CHR(j + ii - 1) 
                                                        | (TypingWord[j + ii - 1] & TONEW_MASK) 
                                                        | (TypingWord[j + ii - 1] & TONE_MASK)
                                                    )
                                                  )
                            ) {
                            _spellingFlag = true;
                    ...
                    if (_spellingFlag 
                        || (k < _spellingEndIndex && !vowelSet[l][0]) 
            ...
            for (int ii = 0; ii < _endConsonantTable.size(); ii++) {
                _spellingFlag = false;
            ...
            if (_spellingOK) {
                if (_index >= 3
                    && CHR(_index-1) == KEY_H
                    && CHR(_index-2) == KEY_C
                    && !((TypingWord[_index-3] & MARK1_DAU_SAC_MASK)
                        || (TypingWord[_index-3] & MARK5_DAU_NANG_MASK)
                        || !(TypingWord[_index-3] & MARK_MASK)))
                {
                    _spellingOK = false;
                }
                else if (_index >= 2 &&
                            CHR(_index-1) == KEY_T
                            && !(  (TypingWord[_index-2] & MARK1_DAU_SAC_MASK)
                                || (TypingWord[_index-2] & MARK5_DAU_NANG_MASK)
                ...
                /*if (_index >= 3) {
                    LOG(L"\t 3 1=%u=%c 2=%u=%c 3=%u=%c dau3=%u=%s | dau2=%u=%s | dau1=%u=%s\n",
                        CHR(_index-1), CHR(_index-1), CHR(_index-2), CHR(_index-2),
                        CHR(_index-3), CHR(_index-3),
                        TypingWord[_index-3], getToneMarkDesc(TypingWord[_index-3]),
                        TypingWord[_index-2], getToneMarkDesc(TypingWord[_index-2]),
                        TypingWord[_index-1], getToneMarkDesc(TypingWord[_index-1])
                        );
                }                    
                else if (_index >= 2) {
                    LOG(L"\t 2 1=%u=%c 2=%u=%c dau2=%u=%s | dau1=%u=%s\n",
                        CHR(_index - 1), CHR(_index - 1), CHR(_index - 2), CHR(_index - 2),
                        TypingWord[_index - 2], getToneMarkDesc(TypingWord[_index - 2]),
                        TypingWord[_index - 1], getToneMarkDesc(TypingWord[_index - 1])
                    );
    ...
    //LOG(L"\t _spellingOK=%u _spellingVowelOK=%d _spellingEndIndex=%u\n", 
    //    _spellingOK, _spellingVowelOK, _spellingEndIndex);
}

static void checkGrammar(const int& deltaBackSpace) {
...
static void insertKey(const Uint16& keyCode, const bool& isCaps, const bool& isCheckSpelling=true) {
    if (_index >= MAX_BUFF) {
        _longWordHelper.push_back(TypingWord[0]); //save long word
        //left shift
        for (size_t iii = 0; iii < MAX_BUFF - 1; iii++) {
...
static void insertState(const Uint16& keyCode, const bool& isCaps) {
    ...
    if (hCode != vReplaceMaro) {
        if (_index > 0) {
            if (_longWordHelper.size() > 0) { //save long word first
                _typingStatesData.clear();
                for (size_t i = 0; i < _longWordHelper.size(); i++) {
        ...
        _typingStatesData.clear();
        for (size_t i = 0; i < hMacroData.size(); i++) {
...
static void saveWord(const Uint32& keyCode, const int& count) {
    _typingStatesData.clear();
...
static void saveSpecialChar() {
    _typingStatesData.clear();
    for (size_t i = 0; i < _specialChar.size(); i++) {
...
static bool existInKeyCode(const Uint16 &val)
{
    static unsigned charKeyCode[] = {
    ...
    for (unsigned i = 0; i < sizeof(charKeyCode) / sizeof(unsigned); ++i)
    {
        if (val == charKeyCode[i])
            return true;
    }
...
static void restoreLastTypingState() {
    if (_typingStates.size() > 0) {
        ...
        if (_typingStatesData.size() > 0){
            if (_typingStatesData[0] == KEY_SPACE) {
                ...
                _index = 0;
            } else if (existInKeyCode( (Uint16)_typingStatesData[0]))  {
                _index = 0;
                _specialChar = _typingStatesData;
                checkSpelling();
            } else {
                for (size_t i = 0; i < _typingStatesData.size(); i++) {
                    TypingWord[i] = _typingStatesData[i];
                }
                _index = (myByte_t)_typingStatesData.size();
            }
        }
    }
}
...
static void checkCorrectVowel(std::vector<std::vector<Uint16>>& charset, int& i, int& k, const Uint16& markKey) {
    //ignore "qu" case
...
Uint32 getCharacterCode(const Uint32& data) {
    ...
    if (data & MARK_MASK) { //has mark
        ...
        switch (data & MARK_MASK) {
            case MARK1_DAU_SAC_MASK:
                markElem = 0;
                break;
            case MARK2_DAU_HUYEN_MASK:
                markElem = 2;
                break;
            case MARK3_DAU_HOI_MASK:
                markElem = 4;
                break;
            case MARK4_DAU_NGA_MASK:
                markElem = 6;
                break;
            case MARK5_DAU_NANG_MASK:
                markElem = 8;
                break;
        }
        markElem += capsElem;
...
static void findAndCalculateVowel(const bool& forGrammar) {
    vowelCount = 0;
    VSI = VEI = 0;
    for (int iii = _index - 1; iii >= 0; --iii) {
...
static void insertMark(const Uint32& markMask, const bool& canModifyFlag) {
    vowelCount = 0;
...
static void insertD(const Uint16& data, const bool& isCaps) {
    hCode = vWillProcess;
    hBPC = 0;
...
static void insertAOE(const Uint16& data, const bool& isCaps) {
    findAndCalculateVowel();
...
static void insertW(const Uint16& data, const bool& isCaps) {
    isRestoredW = false;
...
static void reverseLastStandaloneChar(const Uint32& keyCode, const bool& isCaps) {
    hCode = vWillProcess;
    hBPC = 0;
    hNCC = 1;
    hExt = 4;
...
static void checkForStandaloneChar(const Uint16& data, const bool& isCaps, const Uint32& keyWillReverse) {
    ...
    if (_index == 0) { //zero char
        ...
        for (int i = 0; i < _standaloneWbad.size(); i++) {
        ...
        for (int i = 0; i < _doubleWAllowed.size(); i++) {
...
static void upperCaseFirstCharacter() {
...
static void handleMainKey(const Uint16& data, const bool& isCaps) {
        ...
        for (int i = 0; i < _vowelForMark.size(); i++) {
            ...
            for (int l = 0; l < charset.size(); l++) {
                ...
                if (isCorect) {
                    isChanged = true;
                    if (IS_KEY_S(data))
                        insertMark(MARK1_DAU_SAC_MASK);
                    else if (IS_KEY_F(data))
                        insertMark(MARK2_DAU_HUYEN_MASK);
                    else if (IS_KEY_R(data))
                        insertMark(MARK3_DAU_HOI_MASK);
                    else if (IS_KEY_X(data))
                        insertMark(MARK4_DAU_NGA_MASK);
                    else if (IS_KEY_J(data))
                        insertMark(MARK5_DAU_NANG_MASK);
                    break;
                }
            }
...
static void handleQuickTelex(const Uint16& data, const bool& isCaps) {
    hCode = vWillProcess;
    hBPC = 1;
    hNCC = 2;
...
static bool checkRestoreIfWrongSpelling(const int& handleCode) {
    for (size_t ii = 0; ii < _index; ii++) {
        if (!IS_CONSONANT(CHR(ii)) &&
            ...
            hBPC = _index;
            hNCC = _stateIndex;
            for (size_t i = 0; i < _stateIndex; i++) {
...
static bool checkQuickConsonant() {
...
void vEnglishMode(const vKeyEventState& state, const Uint16& data, const eCapStatus& isCaps, const bool& otherControlKey) {
    hCode = vDoNothing;
    if (state == vKeyEventState::MouseDown || (otherControlKey && !isCaps)) {
        ...
        if (isWordBreak(vKeyEvent::Keyboard, state, data, isCaps) &&
            existInKeyCode(data)) {
            hMacroKey.clear();
            _willTempOffEngine = false;
        } else {
...
static bool isUpperCase(const Uint16& data, const eCapStatus& capsStatus)
{
    if (capsStatus)
    {
        switch (data) {
...
void vKeyHandleEvent(const vKeyEvent& event,
                     ...
                     const Uint16& data,
                     const eCapStatus& capsStatus,
                     const bool& otherControlKey) {
    _isCaps = (capsStatus == eShift || //shift
               capsStatus == eCapLocK); //caps lock
    //if (event != vKeyEvent::Mouse)
    //    LOG(L"\t data=%u capsStatus=%d\n", data, capsStatus);
    if ((IS_NUMBER_KEY(data) && capsStatus == eShift)
        || otherControlKey || isWordBreak(event, state, data, capsStatus)
        || (_index == 0 && IS_NUMBER_KEY(data))) 
    {
        ...
        hNCC = 0;
        hExt = 1; //word break
        //LOG(L"\t data=%u isWordBreak=%d\n", data, isWordBreak(event, state, data, capsStatus));
        //check macro feature
        ...
        else if (vRestoreIfWrongSpelling && isWordBreak(event, state, data, capsStatus))
        ...
        _isCharKeyCode = state == KeyDown && existInKeyCode(data);
        ...
        if (vUpperCaseFirstChar) {
            if (data == KEY_DOT || isUpperCase(data, capsStatus))
                _upperCaseStatus = 1;
    ...
    { //START AND CHECK KEY
        ...
        if (!IS_SPECIALKEY(data) || tempDisableKey) { //do nothing
            if (vQuickTelex && IS_QUICK_TELEX_KEY(data)) {
                ...
                hCode = vDoNothing;
                hBPC = 0;
                hNCC = 0; //LOG(L"\t normal key\n");
                hExt = 3; //normal key
                insertKey(data, _isCaps);
            }
        } else { //check and update key
            //restore state
            hCode = vDoNothing; //LOG(L"\t check and update key\n");
            hExt = 3; //normal key
        ...
        if (vUseMacro) {
            if (hCode == vDoNothing) {
                ...
                for (size_t i = 0; i < hBPC; i++) {
                    if (hMacroKey.size() > 0) {
                        hMacroKey.pop_back();
                    }
                }
                for (size_t i = _index - hBPC; i < hNCC + (_index - hBPC); i++) {
