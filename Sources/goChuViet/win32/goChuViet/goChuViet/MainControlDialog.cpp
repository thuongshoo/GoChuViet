...
void MainControlDialog::initDialog() {
	...
	wsprintfW(titleBuffer, title, goChuVietHelper::getVersionString().c_str());
	SetWindowText(hDlg, titleBuffer);

	//input type
	comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
	std::vector<LPCTSTR>& inputType = goChuVietManager::getInputType();
	...
	std::vector<LPCTSTR>& tableCode = goChuVietManager::getTableCode();
	for (size_t i = 0; i < tableCode.size(); i++) {
	...
	checkTempOffgoChuViet = GetDlgItem(hTabPage1, IDC_CHECK_TEMP_OFF_OPEN_KEY);
	createToolTip(checkTempOffgoChuViet, IDS_STRING_TEMP_OFF_OPENKEY);
	...
	case WM_COMMAND: {
		...
		default:
			if (HIWORD(wParam) == CBN_SELCHANGE) {
				...
				_lastKeyCode = goChuVietManager::_lastKeyCode;
				if (_lastKeyCode > 0) {
					goChuVietManager::_lastKeyCode = 0;
	...
	case WM_NOTIFY: {
		...
		case NM_RETURN: {
			...
			if (link->hdr.idFrom == IDC_SYSLINK_HOME_PAGE)
				...
				ShellExecute(NULL, _T("open"), _T("mailto:yuyoonshoo@gmail.com"), NULL, NULL, SW_SHOWNORMAL);
			break;
		}
		}
		break;
...
void MainControlDialog::fillData() {
	...
	SendMessage(checkTempOffgoChuViet, BM_SETCHECK, vTempOffgoChuViet ? 1 : 0, 0);
...
void MainControlDialog::onCheckboxClicked(const HWND & hWnd) {
	...
	if (hWnd == checkCtrl) {
		...
		val = (eLanguage)SendMessage(checkVietnamese, BM_GETCHECK, 0, 0);
		APP_SET_DATA(vLanguage, val ? eVietnamese : eEnglish);
		...
		if (val)
			goChuVietManager::createDesktopShortcut();
	...
	} else if (hWnd == checkTempOffgoChuViet) {
		val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
		APP_SET_DATA(vTempOffgoChuViet, val ? 1 : 0);
...
void MainControlDialog::requestRestartAsAdmin() {
	goChuVietHelper::registerRunOnStartup(false);
	if (vRunAsAdmin && !IsUserAnAdmin()) {
		int msgboxID = MessageBox(
			hDlg,
			_T("Bạn cần phải khởi động lại GoChuViet để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại GoChuViet không?"),
			_T("GoChuViet"),
			MB_ICONEXCLAMATION | MB_YESNO
		);
		if (msgboxID == IDYES) {
