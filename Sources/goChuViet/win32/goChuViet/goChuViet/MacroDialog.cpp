...
void MacroDialog::saveAndReload() {
	//save
	std::vector<myByte_t> macroData;
	getMacroSaveData(macroData);
	goChuVietHelper::setRegBinary(_T("macroData"), macroData.data(), (int)macroData.size());
...
void MacroDialog::onAddMacroButton() {
	...
	if (name.compare(L"") == 0 || content.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy nhập từ cần gõ tắt!"), _T("GoChuViet"), MB_OK);
		return;
	}
...
void MacroDialog::onDeleteMacroButton() {
	...
	if (name.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy chọn từ cần xoá!"), _T("GoChuViet"), MB_OK);
		return;
	}
...
void MacroDialog::onExportMacrobutton() {
	OPENFILENAME ofn;
	TCHAR szFile[MAX_PATH] = { 'g', 'o', 'C', 'h', 'u', 'V', 'i', 'e', 't', 'v', 'i', 'e', 't', 't', 'a', 't', 0};
	ZeroMemory(&ofn, sizeof(ofn));
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = hDlg;
	ofn.lpstrFile = szFile;
