...
enum eLanguage vLanguage = eVietnamese;
int vInputType = 0;
int vFreeMark = 0;
int vCodeTable = 0;
...
int AppDelegate::run(HINSTANCE hInstance) {
	...
	//init GoChuViet Engine
	goChuVietManager::initEngine();
	...
	while (GetMessage(&msg, nullptr, 0, 0))	{
		if (msg.message == WM_KEYDOWN) {
			goChuVietManager::_lastKeyCode = (UINT16)msg.wParam;
		}
		if (!isDialogMsg(msg)) {
...
void AppDelegate::onDefaultConfig() {
	APP_SET_DATA(vLanguage, eVietnamese);
	...
	APP_SET_DATA(vTempOffgoChuViet, 0);
...
void AppDelegate::onToggleVietnamese() {
	APP_SET_DATA(vLanguage, vLanguage ? eEnglish : eVietnamese);
	if (mainDialog) {
		mainDialog->fillData();
	}
	
	if (vUseSmartSwitchKey) {
		std::string& exe = goChuVietHelper::getLastAppExecuteName();
...
void AppDelegate::ongoChuVietAbout() {
	if (aboutDialog == NULL) {
...
void AppDelegate::ongoChuVietExit() {
	goChuVietManager::freeEngine();
