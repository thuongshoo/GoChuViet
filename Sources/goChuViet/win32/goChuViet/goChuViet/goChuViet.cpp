...
static bool /* _flagChanged = false,*/ _isModifierKey/* a special key occurs such as SHIFT, CAPSLOCK, */;
...
void goChuVietInit() {	
	...
	goChuVietHelper::registerRunOnStartup(vRunWithWindows);
	...
	APP_GET_DATA(vTempOffgoChuViet, 0);
	...
	DWORD macroDataSize;
	BYTE* macroData = goChuVietHelper::getRegBinary(_T("macroData"), macroDataSize);
	initMacroMap((myByte_t*)macroData, (int)macroDataSize);

	//init and load smart switch key data
	DWORD smartSwitchKeySize;
	BYTE* data = goChuVietHelper::getRegBinary(_T("smartSwitchKey"), smartSwitchKeySize);
	initSmartSwitchKey((myByte_t*)data, (int)smartSwitchKeySize);
...
void saveSmartSwitchKeyData() {
	...
	goChuVietHelper::setRegBinary(_T("smartSwitchKey"), savedSmartSwitchKeyData.data(), (int)savedSmartSwitchKeyData.size());
}
...
static void SendKeyCode(Uint32 data) {
	...
	if (!(data & CHAR_CODE_MASK)) {
		...
		if (vCodeTable == 0) { //unicode 2 bytes code
			...
			prepareUnicodeEvent(keyEvent[1], _newChar, false);
			SendInput(2, keyEvent, sizeof(INPUT));
		} else if (vCodeTable == 1 || vCodeTable == 2 || vCodeTable == 4) { //others such as VNI Windows, TCVN3: 1 myByte_t code
...
static void SendBackspace() {
	...
	if (vSupportMetroApp && goChuVietHelper::getLastAppExecuteName().compare("ApplicationFrameHost.exe") == 0) {//Metro App
...
static void SendNewCharString(const bool& dataFromMacro = false) {
	...
	goChuVietHelper::setClipboardText((LPCTSTR)_newCharString.data(), _newCharSize + 1, CF_UNICODETEXT);
...
void switchLanguage() {
	if (vLanguage == eEnglish)
		vLanguage = eVietnamese;
	else
		vLanguage = eEnglish;
	...
	if (vUseSmartSwitchKey) {
		setAppInputMethodStatus(goChuVietHelper::getFrontMostAppExecuteName(), vLanguage | (vCodeTable << 1));
...
static void handleMacro() {
	...
	if (pData->backspaceCount > 0) {
		for (size_t i = 0; i < pData->backspaceCount; i++) {
			SendBackspace();
		}
	}
	//send real data
...
static bool Check_SetModifierMask(const Uint16& vkCode) {
...
static bool UnCheck_SetModifierMask(const Uint16& vkCode) {
	if (vkCode == VK_LSHIFT || vkCode == VK_RSHIFT) _flag &= ~MASK_SHIFT;
	else if (vkCode == VK_LCONTROL || vkCode == VK_RCONTROL) _flag &= ~MASK_CONTROL;
	else if (vkCode == VK_LMENU || vkCode == VK_RMENU) _flag &= ~MASK_ALT;
	else if (vkCode == VK_LWIN || vkCode == VK_RWIN) _flag &= ~MASK_WIN;
...
static enum eCapStatus isShiftOrCaplock(Uint32& flag)
{
	if (_flag & MASK_SHIFT && _flag & MASK_CAPITAL) {
		return eNoCapNoShift;
	}
	else if (_flag & MASK_SHIFT) {
		return eShift;
	}
	else if (_flag & MASK_CAPITAL) {
		return eCapLocK;
	}
	else
		return eNoCapNoShift;
...
LRESULT CALLBACK keyboardHookProcess(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	if (isKeyDown(wParam)) {
		//LOG(L"\nKey down: %d\n", keyboardData->vkCode);
		Check_SetModifierMask((Uint16)keyboardData->vkCode);
	} else if (isKeyUp(wParam)) {
		//LOG(L"\nKey up: %d\n", keyboardData->vkCode);
		UnCheck_SetModifierMask((Uint16)keyboardData->vkCode);
	}
	if (!_isModifierKey && !isKeyUp(wParam))
		_keycode = (Uint16)keyboardData->vkCode;

	//switch language shortcut; convert hotkey
	if (isKeyDown(wParam) && !_isModifierKey && _keycode != 0) {
		...
		else
		{
			...
			if (GET_SWITCH_KEY(convertToolHotKey) == _keycode 
				...
				_hasJustUsedHotKey = true;
				_keycode = 0; //LOG(L"convertToolHotKey\n");
				return -1;
			}
		}
		_hasJustUsedHotKey = _lastFlag != 0;
	}
	else if (_isModifierKey)
	{
		if (_lastFlag == 0 || _lastFlag < _flag)
			...
			if (vTempOffgoChuViet && !_hasJustUsedHotKey && _lastFlag & MASK_ALT) {
				vTempOffEngine();
			}
			_lastFlag = _flag;
			_hasJustUsedHotKey = false;
		}
		_keycode = 0; //LOG(L"CallNextHookEx _lastFlag=%X\n", _lastFlag);
		return CallNextHookEx(hKeyboardHook, nCode, wParam, lParam);
	}

	//if is in english mode
	if (vLanguage == eEnglish) {
		if (vUseMacro && vUseMacroInEnglishMode && isKeyDown(wParam) ) {
			vEnglishMode(( isKeyDown(wParam) ? vKeyEventState::KeyDown : vKeyEventState::MouseDown),
						_keycode,
				        isShiftOrCaplock(_flag),
						OTHER_CONTROL_KEY);
		...
		{ //handle result signal
			...
			if (vFixRecommendBrowser && pData->extCode != 4) {
				if (vFixChromiumBrowser && 
					std::find(_chromiumBrowser.begin(), _chromiumBrowser.end(), goChuVietHelper::getLastAppExecuteName()) != _chromiumBrowser.end()) {
...
VOID CALLBACK winEventProcCallback(HWINEVENTHOOK hWinEventHook, DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
	...
	if (vUseSmartSwitchKey || vRememberCode) {
		std::string& exe = goChuVietHelper::getFrontMostAppExecuteName();
		...
		if (vUseSmartSwitchKey && (_languageTemp & 0x01) != vLanguage) {
			if (_languageTemp != -1) {
				vLanguage = (eLanguage)_languageTemp;
				AppDelegate::getInstance()->onInputMethodChangedFromHotKey();
			} else {
				saveSmartSwitchKeyData();
			}
