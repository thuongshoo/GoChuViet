...
static myByte_t* _regData = 0;

static LPCTSTR sk = TEXT("SOFTWARE\\thuongshoo\\GoChuViet");
static HKEY hKey;
...
static HANDLE _proc;
static std::string _exeNameUtf8 = "ThegoChuVietProject";
...
int CF_OPENKEY = RegisterClipboardFormat(_T("GoChuViet Format"));

void goChuVietHelper::openKey() {
...
void goChuVietHelper::setRegInt(LPCTSTR key, const int & val) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
	RegCloseKey(hKey);
}

int goChuVietHelper::getRegInt(LPCTSTR key, const int & defaultValue) {
	openKey();
...
void goChuVietHelper::setRegBinary(LPCTSTR key, const myByte_t * pData, const int & size) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_BINARY, pData, size);
	RegCloseKey(hKey);
}

BYTE * goChuVietHelper::getRegBinary(LPCTSTR key, DWORD& outSize) {
	openKey();
	if (_regData) {
		delete[] _regData;
		_regData = NULL;
...
void goChuVietHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			std::string path = wideStringToUtf8(getFullPath());
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn GoChuViet /rl highest /tr \"%s\" /f", path.c_str());
			...
			RegSetValueEx(hKey, _T("GoChuViet"), 0, REG_SZ, (BYTE*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("GoChuViet"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn GoChuViet /f", SW_HIDE);
	}
}

LPTSTR goChuVietHelper::getExecutePath() {
	if (!_hasGetPath) {
...
std::string& goChuVietHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("goChuViet64.exe")) == 0 ||
		wcscmp(_exeName, _T("goChuViet32.exe")) == 0 || 
...
std::string & goChuVietHelper::getLastAppExecuteName() {
	if (!vUseSmartSwitchKey)
		return getFrontMostAppExecuteName();
	return _exeNameUtf8;
}

std::wstring goChuVietHelper::getFullPath() {
...
std::wstring goChuVietHelper::getClipboardText(const int& type) {
...
void goChuVietHelper::setClipboardText(LPCTSTR data, const int & len, const int& type) {
...
bool goChuVietHelper::quickConvert() {
...
DWORD goChuVietHelper::getVersionNumber() {
...
std::wstring goChuVietHelper::getVersionString() {
...
std::wstring goChuVietHelper::getContentOfUrl(LPCTSTR url){
	WCHAR path[MAX_PATH];
	GetCurrentDirectory(MAX_PATH, path);
	wsprintf(path, TEXT("%s\\_goChuViet.tempf"), path);
